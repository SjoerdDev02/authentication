use crate::{
    ...
    utils::{
        ...
        redis::{get_token, remove_token, set_token}, responses::AppError,
    },
};
use axum::{
    body::Body,
...
pub async fn jwt_middleware(
    ...
    if let Some(translations) = req.extensions().get::<Translations>() {
        ...
        let claims = match &jwt_cookie {
            Some(token) => decode_jwt(&token).map_err(|_| AppError::format_internal_error(&translations))?,
                ...
                Some(token) => {
                    ...
                    let token_payload_user_id =
                        match get_token(&state, &formatted_refresh_token_key).await {
                            ...
                            Ok(None) => return Err(AppError::format_error(
                                &translations,
                                StatusCode::UNAUTHORIZED,
                                "auth.errors.failed_to_read_token_payload"
                            )),
